#!/usr/bin/env python3
# serverV6.py — DSLR/Mirrorless control on Raspberry Pi (Flask + gphoto2)
# - Live preview (MJPEG) on-demand และ "ส่ง JPEG ตรง ๆ" เพื่อลด CPU
    ...
    for key in lv_keys:
        ...
        if node and node.get_type() in (gp.GP_WIDGET_TOGGLE, gp.GP_WIDGET_RADIO):
            ...
            if truthy is not None and _set_value(cam, cfg, node, truthy):
                enabled = True
    # touch capturetarget เฉย ๆ (บางรุ่นช่วยให้ preview/capture อยู่ร่วมกัน)
    cfg = _cfg(cam)
    if cfg:
        ...
        try:
            if not cam:
                ...
                if cam:
                    ...
                    if supports_preview is None:
                        supports_preview = _check_preview_support(cam)
                    try: try_autofocus(cam, timeout_s=0.3)
                ...
                if supports_preview:
                    try:
                        ...
                        if b and b[:2] == b'\xff\xd8':
                            ...
                            # กล้องบางรุ่นส่งรูปแบบอื่น (เช่น RGB) → ขอข้ามเฟรมนี้
                            pass
                    except gp.GPhoto2Error as e:
...
def api_diag():
    ...
    return jsonify({
        ...
        "mode": mode, "last_error": last_error,
    }), 200
...
def capture():
    ...
    try:
        ...
        if (mime == 'image/jpeg' or host_filepath.lower().endswith('.jpg')) and captured_image[:2] == b'\xff\xd8':
            with lock: latest_frame = captured_image
            pushed = True
        if not pushed:
