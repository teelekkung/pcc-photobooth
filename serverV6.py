#!/usr/bin/env python3
# serverV6.py — DSLR/Mirrorless control on Raspberry Pi (Flask + gphoto2 + OpenCV)
# - Live preview (MJPEG) on-demand: start only when /video_feed has viewers
# - Auto-focus when entering live view and right before capture
# - Broader mirrorless support: try enabling liveview/EVF via multiple keys
...
def connect_camera(camera_port=None):
    ...
    try:
        ...
        try_enable_liveview(cam)  # try enabling live view/EVF for mirrorless/DSLR
        try:
...
# ----------------------------- Config Helpers (robust across brands) -----------------------------
def _cfg(cam):
    try:
...
def _find_child(cfg, names):
    """Find first existing child by trying a list of possible key names."""
    if not cfg:
        return None, None
    for name in names:
        try:
...
def _set_value(cam, cfg, node, value):
    try:
        ...
        print(f"[WARN] set_value({value}) failed for node '{node.get_name() if node else '?'}': {e}")
        return False

# ----------------------------- LiveView / Mirrorless Support -----------------------------
def try_enable_liveview(cam):
    """
    Best-effort enable live view/EVF across DSLR & mirrorless:
    tries a variety of vendor-specific keys.
    """
    cfg = _cfg(cam)
    if not cfg:
        return False

    # Try boolean switches commonly seen
    lv_keys = [
        'viewfinder',         # generic
        'liveview',           # generic/nikon
        'eosviewfinder',      # Canon EOS
        'movie',              # some models gate LV via movie
        'uilock',             # some cameras need UI unlock off
    ]
    ...
    for key in lv_keys:
        ...
        if node and node.get_type() in (gp.GP_WIDGET_TOGGLE, gp.GP_WIDGET_RADIO):
            # Prefer True/"On"/first truthy choice
            truthy = None
            if node.get_type() == gp.GP_WIDGET_TOGGLE:
                truthy = 1
            else:
                # radio: pick something that looks like on/live
                for i in range(node.count_choices()):
                    c = node.get_choice(i).lower()
                    if 'on' in c or 'live' in c or 'enable' in c or 'movie' in c or 'viewfinder' in c:
                        truthy = node.get_choice(i)
                        break
            if truthy is not None:
                if _set_value(cam, cfg, node, truthy):
                    enabled = True

    # Some Nikons/others require 'capturetarget' to 'Memory card' to allow preview/capture interop
    cfg = _cfg(cam)
    if cfg:
        cfg, node = _find_child(cfg, ['capturetarget'])
        if node and node.get_type() == gp.GP_WIDGET_RADIO:
            # Keep whatever works; we just ensure it's set (no-op if already set)
            try:
...
# ----------------------------- Auto Focus -----------------------------
def try_autofocus(cam, timeout_s=1.0):
    """
    Best-effort trigger auto-focus (half-press) across vendors.
    We try several config keys that different brands expose.
    """
    cfg = _cfg(cam)
    if not cfg:
        return False
...
def capture_loop():
    ...
    if cam:
        # Try enabling liveview again just in case, then test preview
        try_enable_liveview(cam)
        if supports_preview is None:
            supports_preview = _check_preview_support(cam)
        # Auto-focus once when entering live mode (helps first frames be sharp)
        try:
            try_autofocus(cam, timeout_s=0.5)
        except Exception as e:
            print(f"[WARN] autofocus on-live failed: {e}")
    ...
    while running:
        try:
            ...
            if mode == "live":
                # If preview not supported, try re-enable liveview once in a while
...
def stop_stream():
    """หยุด live preview ด้วยมือ (ให้เข้ากับ stopCamera() ฝั่ง UI)"""
